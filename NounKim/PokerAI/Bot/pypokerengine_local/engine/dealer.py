...
class Dealer:
  ...
  def __exclude_short_of_money_players(self, table, ante, sb_amount):
    ...
    self.__disable_no_money_player(table.seats.players, self.small_blind_amount)
    table.set_blind_pos(sb_pos, bb_pos)
    if table.seats.players[table.dealer_btn].stack < ante: table.shift_dealer_btn()
    return table
  ...
  def __steal_money_from_poor_player(self, table, ante, sb_amount):
    ...
    # #20220914 annotation script

    # for player in [p for p in players if p.stack < ante]: player.stack = 0
    if players[table.dealer_btn].stack < ante: table.shift_dealer_btn()
  ...
  def __disable_no_money_player(self, players, small_blind_amount):
    #20220914 [== 0] => [< small_blind_amount]
    #print("small_blind_amount: ", small_blind_amount)
    no_money_players = [player for player in players if player.stack < small_blind_amount ]
    no_money_player_names = [player.name for player in no_money_players]
    print("NomoneyPlayers: " , no_money_player_names)
...
class MessageSummarizer(object):
    ...
    def summarize_street_start(self, message):
        base = 'Street "%s" started. (community card = %s) // All Stack: %s // stack: %s'
        stack = { player["name"]:player["stack"] for player in message["round_state"]["seats"] }
        All_stack = sum(stack.values())
        return base % (message["street"], message["round_state"]["community_card"], All_stack, stack)
