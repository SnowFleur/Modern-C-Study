...
streamerServer.on('connection', function (ws, req) {
	...
	console.log("new streamer connect : ", streamer.length);
...
playerServer.on('connection', function (ws, req) {
	...
	if (!streamer[streamer.length - 1] || streamer[streamer.length - 1].readyState != 1 /* OPEN */) {
	...
	ws.on('message', function (msg) {
		...
		if (msg.type == 'offer') {
			...
			streamer[streamer.length - 1].send(JSON.stringify(msg));
		} else if (msg.type == 'iceCandidate') {
	...
	function onPlayerDisconnected() {
		try {
			...
			streamer[streamer.length - 1].send(JSON.stringify({type: 'playerDisconnected', playerId: playerId}));
...
if (config.UseMatchmaker) {
	...
	matchmaker.on('connect', function() {
		...
		message = {
			...
			port: httpPort,
			ready: streamer[streamer.length - 1] && streamer[streamer.length - 1].readyState === 1,
